...
DWORD WINAPI Write(LPVOID lp)
{
	srand(time(0));
	HWND hWnd = HWND(lp);
	HANDLE hEvent = OpenEvent(EVENT_ALL_ACCESS, 0, TEXT("{D0AE4D5D - 1BE3 - 48F7 - 8979 - CFF8738772FE}"));
...
void CriticalSectionDlg::Cls_OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
	if(id == IDC_BUTTON1)
	{
		HANDLE hEvent = CreateEvent(NULL, FALSE , FALSE , TEXT("{D0AE4D5D - 1BE3 - 48F7 - 8979 - CFF8738772FE}"));
